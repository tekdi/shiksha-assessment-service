...
This service uses a **Middleware-based Authorization** pattern. Authenticated user context is provided via request headers. External API Gateways or Middleware are responsible for authentication.

### **Required Headers**
Every request must include:
- `tenantId`: (UUID) Tenant identifier
- `organisationId`: (UUID) Organisation identifier
- `userId`: (UUID) User identifier or `system`

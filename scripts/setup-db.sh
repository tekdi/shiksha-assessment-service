...
# Database Setup Script for Shiksha Assessment Service
...
PGPASSWORD=$DB_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_DATABASE -c "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\"; CREATE EXTENSION IF NOT EXISTS \"pgcrypto\";" > /dev/null 2>&1 || true

# Create tables using schema.sql
echo "🏗️  Creating database tables and constraints..."
if [ -f scripts/schema.sql ]; then
    PGPASSWORD=$DB_PASSWORD psql -h $DB_HOST -p $DB_PORT -U $DB_USERNAME -d $DB_DATABASE -f scripts/schema.sql > /dev/null 2>&1

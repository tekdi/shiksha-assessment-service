...
export class AttemptsService {
  ...
  private async calculateQuestionScore(answerData: any, question: Question): Promise<number> {
    ...
    switch (question.type) {
      ...
      case QuestionType.DROPDOWN:
        score = this.calculateDropdownScore(answerData, question, options);
        break;
        
      case QuestionType.RATING:
        // Use selectedOptionIds if provided, otherwise fall back to rating field
        if (answerData.selectedOptionIds && answerData.selectedOptionIds.length > 0) {
          score = this.calculateRatingScoreWithOptions(answerData, question, options);
        } else if (answerData.rating !== undefined) {
          score = this.calculateRatingScore(answerData, question);
        } else {
          score = 0;
        }
        break;
  ...
  /**
   * Calculates score for dropdown questions
   * Supports both single-select and multi-select dropdowns with partial scoring
   * @param answerData - The parsed user answer data
   * @param question - The question entity with type and marks
   * @param options - Array of question options
   * @returns number - The calculated score for this question
   */
  private calculateDropdownScore(answerData: any, question: Question, options: QuestionOption[]): number {
    const selectedOptionIds = answerData.selectedOptionIds || [];
    ...
    if (selectedOptionIds.length === 1) {
      const selectedOption = options.find(option => option.questionOptionId === selectedOptionIds[0]);
      return selectedOption?.isCorrect ? question.marks : 0;
    }

    // For multi-select dropdown with partial scoring
    if (question.allowPartialScoring) {
      let score = 0;
      for (const optionId of selectedOptionIds) {
        const option = options.find(opt => opt.questionOptionId === optionId);
        if (option?.isCorrect) {
          score += option.marks || 0;
        } else if (option?.isCorrect === false) {
          // Deduct marks for incorrect selections (optional)
          score -= Math.abs(option.marks || 0);
        }
      }
      return Math.max(0, score); // Ensure non-negative score
    }

    // For multi-select dropdown without partial scoring
    const correctSelectedIds = selectedOptionIds.filter(id => 
      correctOptions.some(opt => opt.questionOptionId === id)
    );
    const incorrectSelectedIds = selectedOptionIds.filter(id => 
      !correctOptions.some(opt => opt.questionOptionId === id)
    );

    // All correct options selected and no incorrect ones
    if (correctSelectedIds.length === correctOptions.length && incorrectSelectedIds.length === 0) {
  ...
  /**
   * Calculates score for rating questions
   * Supports different scoring strategies based on rating scale configuration
   * @param answerData - The parsed user answer data
   * @param question - The question entity with type and marks
   * @returns number - The calculated score for this question
   */
  private calculateRatingScore(answerData: any, question: Question): number {
    const rating = answerData.rating;
    const ratingScale = question.params?.ratingScale;
    
    if (rating === undefined || rating === null) {
      return 0;
    }

    // Use default rating scale if not defined
    ...
    // Validate rating follows the step pattern
    if ((rating - min) % step !== 0) {
      return 0; // Invalid rating - doesn't follow step pattern
    }

    // Support rating questions with no marks (0 marks)
    if (question.marks === 0) {
      return 0; // No marks for rating questions with 0 marks
    }

    // Different scoring strategies for rating questions
    const scoringStrategy = question.params?.ratingScale?.scoringStrategy || 'full_marks';
    
    switch (scoringStrategy) {
      case 'full_marks':
        // Full marks for any valid rating (default)
        return question.marks;
        
      case 'proportional':
        // Proportional marks based on rating position in scale
        const totalSteps = Math.floor((max - min) / step) + 1;
        const ratingPosition = Math.floor((rating - min) / step) + 1;
        const proportionalMarks = (ratingPosition / totalSteps) * question.marks;
        return Math.round(proportionalMarks * 100) / 100; // Round to 2 decimal places
        
      case 'threshold':
        // Only give marks if rating meets threshold
        const threshold = question.params?.ratingScale?.threshold || max;
        return rating >= threshold ? question.marks : 0;
        
      case 'target_rating':
        // Full marks only for specific target rating
        const targetRating = question.params?.ratingScale?.targetRating || max;
        return rating === targetRating ? question.marks : 0;
  ...
  /**
   * Calculates score for rating questions with options
   * Uses option marks for different rating levels
   * @param answerData - The parsed user answer data
   * @param question - The question entity with type and marks
   * @param options - Array of question options
   * @returns number - The calculated score for this question
   */
  private calculateRatingScoreWithOptions(answerData: any, question: Question, options: QuestionOption[]): number {
    const selectedOptionIds = answerData.selectedOptionIds || [];
    ...
    // For rating questions, only one option should be selected
    if (selectedOptionIds.length > 1) {
      return 0; // Invalid - rating questions should have single selection
    }
    ...
    // Return the marks assigned to this rating level
    // If option has no marks defined, use question marks
    return selectedOption.marks !== undefined ? selectedOption.marks : question.marks;
  }
} 
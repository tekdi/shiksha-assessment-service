...
export class QuestionsService {
  ...
  private validateDuplicateOptions(createQuestionDto: CreateQuestionDto): void {
    ...
    // For fill-in-the-blank questions, check for duplicate blankIndex values
    if (type === QuestionType.FILL_BLANK) {
      const blankIndices = new Set<number>();
      const duplicateBlankIndices: number[] = [];

      for (const option of options) {
        if (option.blankIndex !== undefined) {
          if (blankIndices.has(option.blankIndex)) {
            duplicateBlankIndices.push(option.blankIndex);
          } else {
            blankIndices.add(option.blankIndex);
          }
        }
      }

      if (duplicateBlankIndices.length > 0) {
        throw new BadRequestException(`Duplicate blankIndex values found: ${duplicateBlankIndices.join(', ')}`);
      }
    }
  }
} 